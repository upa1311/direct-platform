...
test("склонение обязательств и текст успеха", () => {
  ...
  assert.equal(
    formatSettlementSuccess(
      { netDirection: "DIRECT_OWES_RESTAURANT", netAmountCents: 4300, entryCount: 2 },
      "$43.00",
    ),
    "Direct выплатил ресторану: $43.00. Закрыто 2 обязательства.",
  );
  assert.equal(
    formatSettlementSuccess(
      { netDirection: "BALANCED", netAmountCents: 0, entryCount: 1 },
      "$0.00",
    ),
    "Зафиксирован взаимозачёт равных обязательств. Закрыто 1 обязательство.",
  );
});
...
test("подтверждение требует выбора, валидного preview, основания и ссылки при net > 0", () => {
  ...
  // BALANCED допускается без ссылки.
  assert.equal(
    canConfirmSettlement({ ...base, netAmountCents: 0, reference: "" }),
    true,
  );
});
...
test("provider: confirmSettlement под serialized lock с одним nowIso", () => {
  ...
  assert.ok(body.includes("nowIso,"));
