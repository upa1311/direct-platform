...
test("/restaurant/menu использует обычный не-embedded режим", () => {
  ...
  // Роль выводится из режима работы ресторана, а не угадывается по URL.
  assert.ok(MENU_PAGE.includes("resolveMenuWorkspaceRole"));
  assert.ok(MENU_PAGE.includes("SPLIT_OPERATOR_KITCHEN"));
});
