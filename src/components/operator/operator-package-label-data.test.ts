...
const LABEL_CSS = readFileSync(
  new URL("./operator-package-label.module.css", import.meta.url),
  "utf8",
);
...
// Выравнивание и подписи ------------------------------------------------------

test("перед публичным номером есть точная подпись «Номер заказа» без двоеточия", () => {
  assert.ok(LABEL_TSX.includes(">Номер заказа</div>"), "подпись присутствует");
  assert.ok(!LABEL_TSX.includes("Номер заказа:"), "подпись без двоеточия");
  const captionAt = LABEL_TSX.indexOf(">Номер заказа</div>");
  const numberAt = LABEL_TSX.indexOf("{data.publicNumber}");
  assert.ok(captionAt > 0 && numberAt > 0);
  assert.ok(captionAt < numberAt, "подпись идёт перед номером");
});

test("внутренний order.id на наклейку не попадает", () => {
  // Компонент получает только label-data; поля id в ней нет (см. запрещённые
  // ключи), а разметка обращается исключительно к publicNumber.
  assert.ok(!LABEL_TSX.includes("order.id"));
  assert.ok(!LABEL_TSX.includes("data.id"));
  const { order } = acceptedOrder();
  const data = buildOk(order);
  assert.ok(!JSON.stringify(data).includes(order.id));
  assert.equal(data.publicNumber, order.publicNumber);
});

test("текстовая часть сдвинута вправо, платёжный блок — на полной ширине", () => {
  const textBlockAt = LABEL_TSX.indexOf("styles.textBlock");
  const paymentAt = LABEL_TSX.indexOf("styles.payment");
  assert.ok(textBlockAt > 0, "есть контейнер текстовой части");
  assert.ok(textBlockAt < paymentAt, "контейнер идёт до платёжного блока");
  // Платёжный блок не внутри сдвинутого контейнера: контейнер закрывается раньше.
  const textBlockClose = LABEL_TSX.indexOf("</div>", LABEL_TSX.indexOf("styles.addressAccess"));
  assert.ok(textBlockClose < paymentAt, "платёжный блок вне сдвинутого контейнера");
  assert.ok(
    LABEL_CSS.includes("padding-left: 3.5mm"),
    "сдвиг задан отступом контейнера",
  );
  // Логотип не двигается вместе с текстом.
  assert.ok(LABEL_TSX.indexOf("styles.logoBox") < textBlockAt);
});

test("имя клиента и адрес крупнее и жирнее обычной строки", () => {
  for (const cls of ["customer", "address", "addressAccess"]) {
    assert.ok(LABEL_TSX.includes(`styles.${cls}`), `используется класс ${cls}`);
  }
  // Переносы длинных строк сохранены во всех новых классах.
  const blocks = LABEL_CSS.split("}").filter((b) =>
    /\.(customer|address|addressAccess)\s*\{/.test(b),
  );
  assert.equal(blocks.length, 3);
  for (const block of blocks) {
    assert.ok(block.includes("overflow-wrap: anywhere"), block);
    assert.ok(/font-weight: (7[0-9]{2}|800)/.test(block), block);
  }
});
