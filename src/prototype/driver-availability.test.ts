...
test("2: нормализатор принимает схемы 7–18", () => {
  ...
  for (let version = 7; version <= 20; version += 1) {
    ...
    assert.equal(parsed.schemaVersion, 20, `схема ${version} → 18`);
  }
  // Неизвестная будущая версия по-прежнему не принимается.
  assert.equal(
    parseStoredState(JSON.stringify({ ...base, schemaVersion: 21 })),
    null,
  );
});
...
test("34: раздел «Расчёты» водителя существует отдельным маршрутом", () => {
  ...
  assert.ok(
    DRIVER_SETTLEMENTS_PAGE.includes(
      "Здесь будут показаны выполненные доставки, заработок и история расчётов.",
    ),
  );
...
test("35: страница расчётов не показывает выдуманных сумм", () => {
  ...
  assert.ok(!DRIVER_SETTLEMENTS_PAGE.includes("Cents"));
});
