...
/**
 * Финснимок «ресторан должен Direct 800»: единственный источник —
 * канонический moneyMovement. Legacy-поля НАРОЧНО содержат другие суммы:
 * accounting обязан взять 800 из движения, а не 123/4444 из старых полей.
 */
const RESTAURANT_COLLECTED: Partial<FinancialSnapshot> = {
  ...
  platformCommissionReceivableCents: 123,
  restaurantNetAfterPlatformCommissionCents: 4200,
  restaurantPayoutBeforeBankFeeCents: 4444,
  ...
  moneyMovement: {
    ...
    restaurantNetCents: 4200,
    directNetRevenueCents: 800,
  },
};
/**
 * Финснимок «Direct должен ресторану 5100» (после банковской части ресторана
 * 60): legacy-поля нарочно врут (999/5160) — источник только moneyMovement.
 */
const DIRECT_COLLECTED: Partial<FinancialSnapshot> = {
  ...
  platformCommissionReceivableCents: 999,
  restaurantNetAfterPlatformCommissionCents: 5160,
  restaurantPayoutBeforeBankFeeCents: 5160,
  moneyMovementStatus: "COMPLETE",
  moneyMovement: {
    customerMoneyRecipient: "DIRECT",
    paymentChannel: "ONLINE_CARD",
    totalBankFeeCents: 60,
    restaurantBankFeeCents: 60,
    directBankFeeCents: 0,
    restaurantOwesDirectCents: 0,
    directOwesRestaurantCents: 5100,
    restaurantNetCents: 5100,
    directNetRevenueCents: 900,
  },
};
...
test("PICKUP, деньги собрал ресторан → только RESTAURANT_OWES_DIRECT/PLATFORM_COMMISSION", () => {
  ...
  // Сумма — из канонического движения (800), а не из legacy-поля (123).
...
test("RESTAURANT_DELIVERY с оплатой ресторану → комиссия ресторана перед Direct", () => {
  ...
  assert.equal(res.entries[0].type, "PLATFORM_COMMISSION");
  assert.equal(res.entries[0].amountCents, 800);
});
...
test("PLATFORM_DRIVER/онлайн, деньги собрал Direct → DIRECT_OWES_RESTAURANT/RESTAURANT_PAYOUT", () => {
  ...
  assert.equal(res.ok, true);
  assert.equal(res.entries.length, 1);
  assert.equal(res.entries[0].direction, "DIRECT_OWES_RESTAURANT");
  assert.equal(res.entries[0].type, "RESTAURANT_PAYOUT");
  // Выплата уже уменьшена на банковскую часть ресторана (5100, не 5160):
  // старый restaurantPayoutBeforeBankFee/net источником не является.
  assert.equal(res.entries[0].amountCents, 5100);
  assert.equal(
    res.entries[0].amountCents,
    order.financials.moneyMovement?.directOwesRestaurantCents,
  );
  assert.notEqual(
    res.entries[0].amountCents,
...
test("встречные обязательства движения → fail-closed, без двух записей", () => {
  const order = completed("mix", "RESTAURANT_DELIVERY", {
    ...RESTAURANT_COLLECTED,
    moneyMovement: {
      ...RESTAURANT_COLLECTED.moneyMovement!,
      restaurantOwesDirectCents: 500,
      directOwesRestaurantCents: 2500,
    },
  });
  const res = computeCompletedOrderAccounting(order, []);
  ...
  // Recognition тоже fail-closed: state тем же объектом.
  const st = stateWith([order]);
  const rec = recognizeCompletedOrderAccounting(st, "mix", "2026-07-17T12:00:00.000Z");
  assert.equal(rec.result.ok, false);
  assert.equal(rec.state, st);
});

// 5 --------------------------------------------------------------------------

test("сумма строго равна стороне канонического moneyMovement", () => {
  const order = completed("mm1", "PICKUP", RESTAURANT_COLLECTED, "PICKED_UP");
  const res = computeCompletedOrderAccounting(order, []);
  assert.equal(res.ok, true);
  assert.equal(
    res.entries[0].amountCents,
...
test("изменение тарифов, меню и комиссии не меняет записи", () => {
  ...
  const before = computeCompletedOrderAccounting(order, []);
  ...
  const after = computeCompletedOrderAccounting(orderAfter, []);
  assert.deepEqual(after, before);
  assert.equal(after.entries[0].amountCents, 800);
});

// 6.1 — статусы движения и повреждённые состояния ------------------------------

test("PENDING/REVIEW не создают запись; COMPLETE без движения — fail-closed", () => {
  // Завершённый заказ с PENDING-каналом (повреждённая комбинация) — записи нет.
  const pending = completed("mm-p", "PICKUP", {
    ...RESTAURANT_COLLECTED,
    moneyMovementStatus: "PENDING_PAYMENT_CHANNEL",
    moneyMovement: undefined,
  }, "PICKED_UP");
  const pendingRes = computeCompletedOrderAccounting(pending, []);
  assert.equal(pendingRes.ok, true);
  assert.equal(pendingRes.entries.length, 0);

  // REVIEW_REQUIRED: правдоподобная сумма из старых полей не создаётся.
  const review = completed("mm-r", "PICKUP", {
    ...RESTAURANT_COLLECTED,
    moneyMovementStatus: "REVIEW_REQUIRED",
    moneyMovement: undefined,
  }, "PICKED_UP");
  const reviewRes = computeCompletedOrderAccounting(review, []);
  assert.equal(reviewRes.ok, true);
  assert.equal(reviewRes.entries.length, 0);

  // COMPLETE без движения — повреждённое состояние.
  const broken = completed("mm-b", "PICKUP", {
    ...RESTAURANT_COLLECTED,
    moneyMovement: undefined,
  }, "PICKED_UP");
  const brokenRes = computeCompletedOrderAccounting(broken, []);
  assert.equal(brokenRes.ok, false);
  assert.equal(brokenRes.entries.length, 0);
});

test("нулевое движение не создаёт запись; конфликт существующей — fail-closed", () => {
  // Обе стороны 0 — обязательств нет.
  const zero = completed("mm-z", "PICKUP", {
    ...RESTAURANT_COLLECTED,
    moneyMovement: {
      ...RESTAURANT_COLLECTED.moneyMovement!,
      restaurantOwesDirectCents: 0,
      directOwesRestaurantCents: 0,
      restaurantNetCents: 5000,
      directNetRevenueCents: 0,
    },
  }, "PICKED_UP");
  const zeroRes = computeCompletedOrderAccounting(zero, []);
  assert.equal(zeroRes.ok, true);
  assert.equal(zeroRes.entries.length, 0);

  // Существующая запись с другой суммой: ошибка, а не тихая замена.
  const order = completed("mm-c", "PICKUP", RESTAURANT_COLLECTED, "PICKED_UP");
  const okFirst = computeCompletedOrderAccounting(order, []);
  assert.equal(okFirst.ok, true);
  const conflicting = { ...okFirst.entries[0], amountCents: 801 };
  const conflictRes = computeCompletedOrderAccounting(order, [conflicting]);
  assert.equal(conflictRes.ok, false);
  assert.ok(/противоречит/.test(conflictRes.error ?? ""));
  // Полное совпадение — идемпотентный no-op.
  const sameRes = computeCompletedOrderAccounting(order, okFirst.entries);
...
test("PREPARING/READY/CANCELED не создают записей", () => {
  for (const status of ["PREPARING", "READY", "READY_FOR_PICKUP", "CANCELED"] as const) {
    ...
    assert.equal(computeCompletedOrderAccounting(order, []).entries.length, 0, status);
...
test("гонка двух вкладок оставляет ровно одну запись заказа", () => {
  const order = completed("race", "RESTAURANT_DELIVERY", RESTAURANT_COLLECTED);
  ...
  // Один обычный заказ — максимум одна accounting entry.
  assert.equal(forOrder.length, 1);
  assert.equal(forOrder[0].amountCents, 800);
});
...
test("PICKUP completion + serialize/parse → ровно одна PLATFORM_COMMISSION", () => {
  ...
  // Сумма — из канонического движения (800), не из legacy-поля (123).
  assert.equal(getRestaurantOpenReceivableCents(parsed, RESTAURANT_ID), 800);
});

// 16.1 -----------------------------------------------------------------------

test("pickup card создаёт тот же долг Direct, что и cash: банк не увеличивает entry", () => {
  // Свежий pickup-снимок (PENDING): выдача фиксирует фактический канал и
  // рассчитывает движение канонически.
  const makeReady = (id: string): Order => ({
    ...makeOrder({
      id,
      status: "READY_FOR_PICKUP",
      deliveryMode: "PICKUP",
      paymentStatus: "DUE_AT_PICKUP",
      paidAt: null,
      fin: { moneyMovementStatus: "PENDING_PAYMENT_CHANNEL", moneyMovement: undefined },
    }),
    paymentMethod: "PAY_AT_RESTAURANT",
    pickupCodeUsed: false,
    pickupPaymentMethodsSnapshot: ["CASH", "CARD"],
  });
  const cash = completePickupWithCode(
    stateWith([makeReady("mm-cash")]),
    "mm-cash", "0000", "CASH", "RESTAURANT", DELIVERED_AT, "COMBINED",
  );
  const card = completePickupWithCode(
    stateWith([makeReady("mm-card")]),
    "mm-card", "0000", "CARD", "RESTAURANT", DELIVERED_AT, "COMBINED",
  );
  assert.equal(cash.result.ok, true, cash.result.error ?? "");
  assert.equal(card.result.ok, true, card.result.error ?? "");
  const cashEntry = cash.state.restaurantAccountingEntries.find((e) => e.orderId === "mm-cash");
  const cardEntry = card.state.restaurantAccountingEntries.find((e) => e.orderId === "mm-card");
  assert.ok(cashEntry && cardEntry);
  const cardMovement = card.state.orders.find((o) => o.id === "mm-card")!
    .financials.moneyMovement;
  assert.ok(cardMovement);
  // Карта несёт банковскую комиссию, но долг перед Direct — та же комиссия.
  assert.ok(cardMovement.restaurantBankFeeCents > 0);
  assert.equal(cardEntry.amountCents, cashEntry.amountCents);
  assert.equal(cardEntry.amountCents, cardMovement.restaurantOwesDirectCents);
  assert.equal(cardEntry.direction, "RESTAURANT_OWES_DIRECT");
});
...
test("повторный parse не растит число записей и receivable", () => {
  ...
  // Сумма — из канонического движения (800), не из legacy-поля (123).
  assert.equal(getRestaurantOpenReceivableCents(parse2, RESTAURANT_ID), 800);
});
...
test("подтверждение orphan: грамматически корректно, без «по заказу» и без id", () => {
  ...
  assert.equal(
    settled,
    "Расчёт по старому начислению на сумму $3.00 зафиксирован. Денежный перевод системой не выполнялся.",
  );
  assert.ok(!settled.includes("по заказу"), "не «по заказу Старое начисление»");
  ...
  assert.equal(
    waived,
    "Старое комиссионное начисление Direct на сумму $3.00 списано. Это не возврат и не выплата ресторану.",
  );
  assert.ok(!waived.includes("по заказу"));
});
