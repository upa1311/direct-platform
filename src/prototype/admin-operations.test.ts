...
// 5: PLATFORM_DRIVER может получить доступного водителя
...
test("завершение заказа освобождает водителя", () => {
  ...
  const assigned = assignDriverToOrder(state, orderId, "driver-1");
  const delivered = markOrderDeliveredByDriver(assigned.state, orderId);
  assert.equal(
...
test("выдача PICKUP без кода невозможна обычным действием", () => {
  const { state, orderId } = makePickupReady();
  const wrong = completePickupWithCode(state, orderId, "0000");
...
test("аварийная выдача без кода требует причину", () => {
  const { state, orderId } = makePickupReady();
  const res = issuePickupWithoutCode(state, orderId, "  ");
  assert.equal(res.result.ok, false);
...
test("аварийная выдача создаёт одну settlement и не дублирует", () => {
  const { state, orderId } = makePickupReady();
  const first = issuePickupWithoutCode(state, orderId, "Клиент забыл код");
  ...
  const second = issuePickupWithoutCode(first.state, orderId, "Ещё раз");
