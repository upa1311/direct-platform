...
): ReturnType<typeof buildRestaurantSettlementOverview> {
...
test("16: legacy-строка без движения работает и помечается LEGACY", () => {
...
test("18: дневная сверка строится без ошибок и сходится с итогами", () => {
  ...
  const days = buildRestaurantDailySettlement(
    state,
    "restaurant-2",
    "ALL",
    new Date().toISOString(),
