...
test("DIRECT финансы: PICKUP обнуляет доставку, выплату и доплату", () => {
  // Малая сумма (еда 400 < порога): доплата всё равно 0 для PICKUP.
  const r = computeDirectFinancials({
  ...
  assert.equal(r.smallOrderFeeCents, 0, "PICKUP доплату не получает");
  assert.equal(r.platformGrossRevenueCents, r.restaurantCommissionCents);
  assert.equal(r.customerTotalCents, 400, "итог клиента = только еда");
});
...
test("самовывоз: small-order fee НЕ применяется даже при малой сумме", () => {
  // Еда $4.00 (ниже порога), комиссия 15% = 60. Доплата к PICKUP не начисляется:
  // итог клиента = только еда, задолженность Direct = только комиссия.
  const s = computePickupSettlement({
    foodSubtotalCents: 400,
    commissionRateBps: 1500,
  });
  assert.equal(s.restaurantCommissionCents, 60);
  assert.equal(s.customerTotalCents, 400); // только еда, без доплаты и доставки
  assert.equal(s.platformCommissionReceivableCents, 60); // только комиссия
