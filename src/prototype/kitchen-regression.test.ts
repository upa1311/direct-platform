...
/**
 * Снимок без канонического движения денег (v10): рассчитанные суммы заказа
 * неизменны на всех шагах, а movement легитимно ДОПОЛНЯЕТСЯ выдачей
...
function invariantsOf(state: PrototypeState, orderId: string) {
  ...
  return {
    financials: financialsWithoutMovement(o),
...
function expectStep(
  ...
  assert.equal(financialsWithoutMovement(order), before.financials);
...
test("Regression 1: COMBINED PICKUP — приём → готовность → выдача по коду", () => {
  ...
  const baseFinancials = financialsWithoutMovement(created);
  ...
  assert.equal(financialsWithoutMovement(final), baseFinancials);
...
test("Regression 2: COMBINED PICKUP — невыкуп после 30 минут вместо выдачи", () => {
  ...
  const baseFinancials = financialsWithoutMovement(readyOrder);
  ...
  assert.equal(financialsWithoutMovement(o), baseFinancials);
...
test("Regression 3: SPLIT PICKUP — оператор принимает, кухня готовит, выдаёт оператор", () => {
  const { state, orderId } = makeOrder("restaurant-2", "PICKUP", "SPLIT_OPERATOR_KITCHEN");

  // Решение по новому заказу — зона оператора: кухня не принимает.
  expectRejected(state, acceptRestaurantOrderWithResult(state, orderId, 20, "RESTAURANT", "KITCHEN"), orderId);
  const accepted = acceptRestaurantOrderWithResult(state, orderId, 20, "RESTAURANT", "OPERATOR");
  assert.equal(accepted.result.ok, true);
  expectStep(state, accepted.state, orderId, {
    from: "RESTAURANT_REVIEW", to: "PREPARING", type: "STATUS",
