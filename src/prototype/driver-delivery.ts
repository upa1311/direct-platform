 ...
 *
 * v24: наличное завершение через applyDriverDeliveredOrder создаёт обязательство
 * ресторана RESTAURANT_REMITTANCE (ресторан получил наличные от водителя и должен
 * Direct комиссию + small-order fee). Сумма берётся ТОЛЬКО из канонического
 * движения денег снимка; стоимость доставки в долг ресторана не входит (её
 * водитель уже удержал). Получение денег, заработок водителя и обязательство
 * ресторана сохраняются одной атомарной мутацией.
 */
