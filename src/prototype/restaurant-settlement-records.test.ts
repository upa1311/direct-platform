...
test("preview отклоняет несовместимую пару направление/основание", () => {
  ...
  // Валидная запись сама по себе по-прежнему считается.
  const onlyValid = okPreview(st, ["c1"]);
  ...
  const confirmed = confirmRestaurantSettlement(
    st,
    RID,
    ["w1"],
    "Основание",
...
test("confirm: BALANCED допускает пустую ссылку, ненулевой итог — нет", () => {
  ...
  const balanced = confirmRestaurantSettlement(
    st,
    RID,
    ["c1", "p1"],
  ...
  const noRef = confirmRestaurantSettlement(
    single,
    RID,
    ["c1"],
    "Основание есть",
...
test("confirm: любая ошибка не меняет state и не закрывает обязательства", () => {
  ...
  const cases: [string, () => ReturnType<typeof confirmRestaurantSettlement>][] = [
    ["без основания", () => confirmRestaurantSettlement(st, RID, ["c1"], "   ", "ref", NOW)],
    ["плохое время", () => confirmRestaurantSettlement(st, RID, ["c1"], "ок", "ref", "не-дата")],
    ["несуществующий entry", () => confirmRestaurantSettlement(st, RID, ["c1", "нет"], "ок", "ref", NOW)],
    ["дубли", () => confirmRestaurantSettlement(st, RID, ["c1", "c1"], "ок", "ref", NOW)],
  ];
...
test("частичный расчёт оставляет невыбранные обязательства открытыми", () => {
  ...
  const res = confirmRestaurantSettlement(st, RID, ["c1"], "Частичный", "ref", NOW);
  assert.equal(res.result.ok, true, res.result.error ?? "");
...
test("старое состояние мигрирует с пустым массивом записей", () => {
  ...
  assert.equal(PROTOTYPE_SCHEMA_VERSION, 12);
