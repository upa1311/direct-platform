...
test("cash-31: схема поднята до 21", () => {
  assert.equal(PROTOTYPE_SCHEMA_VERSION, 21);
});
...
test("cash-ui-51: нет текста ещё не реализованных cash-этапов (клиент/долг/ledger)", () => {
  // Передача ресторану реализована в части 3, поэтому её строки допустимы.
  // Запрещены только будущие этапы: получение денег от клиента, долг, ledger.
  for (const forbidden of [
    "Получить от клиента",
    "Наличные получены от клиента",
    "Долг водителя",
    "ledger",
  ]) {
