...
test("77–78: /driver/offers больше не заглушка и не содержит «рабочая смена»", () => {
  assert.ok(!OFFERS_PAGE.includes("SectionPanel"));
  assert.ok(!OFFERS_PAGE.includes("PageHeading"));
  assert.ok(OFFERS_PAGE.includes("Предложений пока нет"));
  assert.ok(!/рабоч[аеу]\w*\s+смен/i.test(OFFERS_PAGE));
  assert.ok(!/рабоч[аеу]\w*\s+смен/i.test(DRIVER_PAGE));
});

test("79–81: до принятия видны улица и зоны, но не точный адрес и телефон", () => {
  assert.ok(OFFERS_PAGE.includes("order.address?.street"));
  assert.ok(OFFERS_PAGE.includes("zoneName(order.restaurant.zoneId)"));
  assert.ok(OFFERS_PAGE.includes("zoneName(order.financials.customerZoneId)"));
  // Точный адрес и контакты клиента на странице предложений не читаются.
  for (const forbidden of [
    ...
    assert.ok(!OFFERS_PAGE.includes(forbidden), `offers: ${forbidden}`);
  }
});

test("82: выплата берётся из сохранённого снимка", () => {
  assert.ok(OFFERS_PAGE.includes("order.financials.driverPayoutCents"));
  assert.ok(OFFERS_PAGE.includes("formatMoney("));
});

test("83–86: countdown, «Принять заказ», «Отказаться», без причины отказа", () => {
  assert.ok(OFFERS_PAGE.includes("Осталось:"));
  assert.ok(OFFERS_PAGE.includes("formatCountdown"));
  assert.ok(OFFERS_PAGE.includes("Принять заказ"));
  assert.ok(OFFERS_PAGE.includes("Отказаться"));
...
test("90: сырые статусы предложений в UI не печатаются", () => {
  for (const page of [OFFERS_PAGE, CURRENT_PAGE, DRIVER_PAGE]) {
