...
test("Accept против auto-expire: кухня приняла первой — sweep не отменяет заказ", () => {
  const { state: initial, orderId } = splitOrderState("PICKUP");
  const store = makeSharedStore(initial);
  const kitchenTab: Tab = { local: initial };
...
test("Accept против auto-expire: sweep отменил первым — устаревшая кухня получает ошибку", () => {
  const { state: initial, orderId } = splitOrderState("PICKUP");
  const store = makeSharedStore(initial);
  const kitchenTab: Tab = { local: initial };
  const sweepTab: Tab = { local: initial };
...
test("Accept против client cancel: клиент отменил первым — устаревшая кухня получает ошибку", () => {
  const { state: initial, orderId } = splitOrderState("PICKUP");
  const store = makeSharedStore(initial);
  const kitchenTab: Tab = { local: initial };
...
test("Accept против client cancel: кухня приняла первой — устаревший клиент не отменяет бесплатно", () => {
  const { state: initial, orderId } = splitOrderState("PICKUP");
  const store = makeSharedStore(initial);
  const kitchenTab: Tab = { local: initial };
  const clientTab: Tab = { local: initial };
...
test("Stale cart mutation после принятия: rebase сохраняет и заказ, и корзину", () => {
  ...
  store.runUnderLock(kitchenTab, (base) =>
    acceptRestaurantOrderWithResult(base, orderId, 20, "RESTAURANT", "KITCHEN"),
  );
...
test("No-op мутация: ничего не записывает, не растит ревизию, принимает свежий base", () => {
  const { state: initial, orderId } = splitOrderState("PICKUP");
  const accepted = acceptRestaurantOrder(initial, orderId, 20, "RESTAURANT", "KITCHEN");
...
test("Pickup code не используется дважды; settlement не дублируется", () => {
  const { state: initial, orderId } = splitOrderState("PICKUP");
  let s = acceptRestaurantOrder(initial, orderId, 20, "RESTAURANT", "KITCHEN");
