...
test("Accept против client cancel: оператор принял, кухня не начала — неоплаченный заказ отменяется напрямую", () => {
  // SPLIT PICKUP: после принятия kitchenStartedAt ещё null, заказ не оплачен —
  // прямая клиентская отмена разрешена (возврат денег не нужен). Fail-closed
  // гонка перенесена на фактический старт кухни (см. следующий тест).
  const { state: initial, orderId } = splitOrderState("PICKUP");
  const store = makeSharedStore(initial);
  const acceptingTab: Tab = { local: initial };
  const clientTab: Tab = { local: initial };
...
test("Kitchen start против client cancel: кухня начала первой — устаревший клиент не отменяет напрямую", () => {
  const { state: initial, orderId } = splitOrderState("PICKUP");
  const store = makeSharedStore(initial);
  const acceptingTab: Tab = { local: initial };
  ...
  assert.equal(acceptRes.ok, true);
  const startRes = store.runUnderLock(kitchenTab, (base) =>
