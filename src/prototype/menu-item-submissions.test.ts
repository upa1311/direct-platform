...
test("права: COMBINED и кухня в SPLIT ведут каталог, оператор — нет", () => {
  // COMBINED.
  ...
  // Оператор в SPLIT — нет.
  const splitState = baseState("SPLIT_OPERATOR_KITCHEN");
  assertUnchanged(
    splitState,
    createMenuItemSubmissionDraft(splitState, RID, "RESTAURANT", "OPERATOR"),
  );

  // SPLIT без роли — fail-closed.
  assertUnchanged(
    splitState,
...
test("чужой ресторан не может править заявку", () => {
  const { state, submissionId } = draftReadyToSubmit();
  // Ресторан-2 переводим в SPLIT и пробуем править чужую заявку его кухней:
  // проверка идёт по ресторану-владельцу заявки, поэтому роль не помогает.
  const foreign = {
    ...state,
    restaurants: state.restaurants.map((r) =>
